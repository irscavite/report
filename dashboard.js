...
export async function calculateLineStats() {
    ...
    visibleRows.forEach(tr => {
        const rowCategory = getStatusText(tr).toLowerCase();
        const isDamageRow = rowCategory === 'damage' || rowCategory === 'dmg';
        const hasApp = tr.cells[11].innerText.trim() !== "";
        const hasRep = tr.cells[13].innerText.trim() !== "";

        // REPAIRED must follow the active filter/search and count every visible row
        // that has a repaired date. Do not limit this to Damage only, because when
        // FILTER BY = REPAIR the expected tally is all filtered rows with repair date.
        if (hasRep) rep++;

        // Approval and pending counts stay DAMAGE-only. Available rows should not
        // inflate Remaining Pending or Remaining Approved.
