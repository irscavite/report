...
const PDF_REPORT_TYPES = [
    "Repo Container Monitoring Report",
    "Container Movement Timeline",
    "Pending Approved Containers Report",
    "Repaired Containers Report"
];
...
function createExportModalIfMissing() {
    ...
    modal.innerHTML = `
        <div class="export-modal-box">
            ...
            <div class="export-report-type-wrap" id="export-report-type-wrap" style="display:none;">
                <label for="export-report-type">Report Type</label>
                <select id="export-report-type">
                    ${PDF_REPORT_TYPES.map(title => `<option value="${title}">${title}</option>`).join("")}
                </select>
                <small>The selected report type will be used as the PDF header title.</small>
            </div>
...
function showExportColumnModal(format) {
    ...
    const reportTypeWrap = document.getElementById("export-report-type-wrap");
    const reportTypeSelect = document.getElementById("export-report-type");
    const isPdf = String(format).toLowerCase() === "pdf";

    title.innerText = isPdf ? "Select report type and columns" : `Select columns for ${format.toUpperCase()}`;
    generateBtn.innerText = `Generate ${format.toUpperCase()}`;
    if (reportTypeWrap) reportTypeWrap.style.display = isPdf ? "block" : "none";
    if (reportTypeSelect) reportTypeSelect.value = PDF_REPORT_TYPES[0];
    ...
    return new Promise(resolve => {
        ...
        generateBtn.onclick = () => {
            ...
            if (isPdf) {
                close({
                    selectedKeys,
                    reportType: reportTypeSelect ? reportTypeSelect.value : PDF_REPORT_TYPES[0]
                });
                return;
            }
...
export async function exportPDF() {
    const exportOptions = await showExportColumnModal("pdf");
    if (!exportOptions) return;

    const selectedKeys = exportOptions.selectedKeys || exportOptions;
    const selectedReportType = exportOptions.reportType || PDF_REPORT_TYPES[0];
    ...
    const reportTitle = selectedReportType.toUpperCase();
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.text("Container Monitoring and Repair Management System", marginX, 49);
    ...
    doc.setFontSize(12);
    doc.setTextColor(10, 35, 80);
    doc.text(selectedReportType.toUpperCase(), marginX, 116);
    ...
    const fileReportType = selectedReportType.replace(/[^a-z0-9]+/gi, "_").replace(/^_+|_+$/g, "");
    doc.save(`${fileReportType}_${selectedLine}_${selectedCategory}_${new Date().toISOString().slice(0,10)}.pdf`);
}
